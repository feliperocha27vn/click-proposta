...
export function setupExpiredPlansJob() {
  ...
  cron.schedule('0 9 * * *', async () => {
    console.log(
      '[Cron Job] Iniciando verificação de planos expirados e notificações na API...'
    )
    ...
    try {
      ...
      console.log(
        `[Cron Job] Encontrados ${usersToWarn.length} usuários para aviso prévio na API.`
      )
      ...
      for (const user of usersToWarn) {
        ...
        console.log(`[Cron Job] Aviso enviado via API para: ${user.phone}`)
      }
      ...
      console.log(
        `[Cron Job] Encontrados ${expiredUsers.length} usuários com plano expirado na API.`
      )
      ...
      for (const user of expiredUsers) {
        ...
        await messagingProvider.sendMessage(user.phone, message)
        console.log(
          `[Cron Job] Notificação de expiração enviada via API e plano resetado para: ${user.phone}`
        )
      }
    } catch (error) {
      console.error(
        '[Cron Job] Erro ao processar expiração de planos na API:',
        error
      )
    }
  })
  ...
  console.log(
    '[Cron Job] Job de verificação de planos agendado na API (09:00 daily).'
  )
}
